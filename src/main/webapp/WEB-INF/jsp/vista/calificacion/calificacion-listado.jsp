	...
	<section class="midBot bgc1 p-5">
		...
		<table class="table">
			...
			<c:forEach var="alumno" items="${alumnos}">
				<tr>
					<td><c:out value="${calificacion.numeroEvaluacion}" /></td>
					<td><c:out value="${estudiante.nombre}" /></td>
					<td><c:out value="${estudiante.calificacion.asignatura}" /></td>
					<td><c:out value="${estudiante.calificacion.asignatura}" /></td>
					<td><c:out value="${estudiante.nota}" /></td>
					<td>
						<a href="${pageContext.request.contextPath}/CalificacionController?accion=editar&amp;id=${asignatura.id}">Modificar</a> <%-- contextPath en este caso es /java-people --%>
					</td>
				</tr>
			</c:forEach>			
			</tbody>
